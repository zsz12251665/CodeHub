...
struct node
{
	int i,x,y;// i is the index, x and y is the coodinate of the robot
};
node a[200001];
void qsort(int l,int r)// Sort in increasing order of (x,y,i)
{
	int x=l,y=r;
	node std=a[(l+r)/2];
	while(x<y)
...
int main()
{
	...
	for(scanf("%d",&t);t>0;--t)
	{
		...
		qsort(0,n);
		int ans=-1;// ans is the index of the answer in the array, -1 means there are not answer yet
		for(int i=0;i<n;++i)
