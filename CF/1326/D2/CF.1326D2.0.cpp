...
int main()
{
	...
	{
		scanf("%s",&s);
		int len_s=strlen(s),p=-1;// p==-1 means the string is a palindrome
		for(int i=0;i<len_s-1-i;++i)// Find the length of the longest symmetric prefix and suffix of the substring of s
			if(s[i]!=s[len_s-1-i])
			{
				p=i;
				break;
		...
		int len_t=1;// Forming the string tmp
		tmp[0]='#';
		for(int i=p;i<len_s-p;++i)
		{
			tmp[len_t++]=s[i];
			tmp[len_t++]='#';
		}
		for(int i=0,mx=0,ce;i<len_t;++i)// Calculate the array r
		{
		...
		for(int i=0;i<len_t/2;++i)// Enumerate the length
		{
			if(r[i]==i+1)
				max_pre=i;
			if(r[len_t-1-i]==i+1)
		...
		else
		{
			for(int i=0;i<p;++i)
				putchar(s[i]);
			for(int i=len_s-p-max_suf;i<len_s;++i)
				putchar(s[i]);
		}
		putchar('\n');
	}
