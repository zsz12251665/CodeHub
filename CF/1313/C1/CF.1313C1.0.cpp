...
int main()
{
	...
	for(int i=0;i<n;++i)// Enumerate the peaks
	{
		...
		int min_a=a[i];// The minimum value of a[i], used to make both sides of the peak monotonic but remained the array unchanged
		for(int j=i-1;j>=0;--j)
		{
	...
	for(int i=ans_i-1;i>=0;--i)// Shape the answer
