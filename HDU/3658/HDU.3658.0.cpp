		...
		for (; m > 0; m /= 2) // Calculate the power of matrix
		{
			if (m % 2 == 1)
			{
				...
				for (int i = 0; i < 104; ++i)
					for (int j = 0; j < 104; ++j)
						for (int c = 0; c < 104; ++c)
							temp[i][j] = (answer[i][c] * power[c][j] + temp[i][j]) % MOD;
				for (int i = 0; i < 104; ++i)
					for (int j = 0; j < 104; ++j)
						answer[i][j] = temp[i][j];
			}
			...
			for (int i = 0; i < 104; ++i)
				for (int j = 0; j < 104; ++j)
					for (int c = 0; c < 104; ++c)
						temp[i][j] = (power[i][c] * power[c][j] + temp[i][j]) % MOD;
			for (int i = 0; i < 104; ++i)
				for (int j = 0; j < 104; ++j)
					power[i][j] = temp[i][j];
		}
		int sum = 0; // Calculate the answer, which is the sum of a sub-matrix in the answer matrix
